...
func NewStealCmd() *cobra.Command {
	...
	persistentFlags.IntVar(&opts.concurrency, "concurrency", 4, "Sets the amount of dumps to be performed concurrently")
	persistentFlags.Int64Var(&opts.batch, "batch", 50000, "Sets the batch of dumps to be performed")
	persistentFlags.DurationVar(&opts.readOpts.timeout, "read-timeout", 50*time.Minute, "Sets the timeout for read operations")
	...
	persistentFlags.DurationVar(&opts.writeOpts.timeout, "write-timeout", 50*time.Minute, "Sets the timeout for write operations")
